# Task Index

## Layout

- [`todo/index.md`](todo/index.md): active task queue
- [`done/index.md`](done/index.md): completed task archive

## Status Summary

- Active tasks: 0
- Completed tasks: 9

## Active Tasks

No active tasks. Create a new task spec under [`todo/`](todo/) before starting the next workstream.

## Completed Tasks

- [`done/01-fix-installed-hook-root-resolution.md`](done/01-fix-installed-hook-root-resolution.md)
- [`done/02-honor-no-external-for-srcset.md`](done/02-honor-no-external-for-srcset.md)
- [`done/03-validate-cli-numeric-flags.md`](done/03-validate-cli-numeric-flags.md)
- [`done/04-improve-html-link-extraction.md`](done/04-improve-html-link-extraction.md)
- [`done/05-tune-browser-dead-element-detection.md`](done/05-tune-browser-dead-element-detection.md)
- [`done/06-reduce-non-html-download-cost.md`](done/06-reduce-non-html-download-cost.md)
- [`done/07-establish-release-baseline.md`](done/07-establish-release-baseline.md)
- [`done/08-package-cli-and-installers.md`](done/08-package-cli-and-installers.md)
- [`done/09-publish-first-release.md`](done/09-publish-first-release.md)
