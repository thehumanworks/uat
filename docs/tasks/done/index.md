# Completed Tasks

All task specs in this directory represent completed work and remain here as the historical source of truth for why the change was made, how it was implemented, and how it was verified.

- [`01-fix-installed-hook-root-resolution.md`](01-fix-installed-hook-root-resolution.md)
- [`02-honor-no-external-for-srcset.md`](02-honor-no-external-for-srcset.md)
- [`03-validate-cli-numeric-flags.md`](03-validate-cli-numeric-flags.md)
- [`04-improve-html-link-extraction.md`](04-improve-html-link-extraction.md)
- [`05-tune-browser-dead-element-detection.md`](05-tune-browser-dead-element-detection.md)
- [`06-reduce-non-html-download-cost.md`](06-reduce-non-html-download-cost.md)
- [`07-establish-release-baseline.md`](07-establish-release-baseline.md)
- [`08-package-cli-and-installers.md`](08-package-cli-and-installers.md)
- [`09-publish-first-release.md`](09-publish-first-release.md)
- [`10-refresh-package-readme-and-republish.md`](10-refresh-package-readme-and-republish.md)
